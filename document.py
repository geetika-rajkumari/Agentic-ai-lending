...
from compliance import ComplianceChecker, ApplicantData, KycData, ComplianceInput, ComplianceExtractionOutput
...
class ApplicationSummary(BaseModel):
    ...
    complianceReport: Optional[Dict[str, Any]] = Field(None, description="Compliance report from the compliance checker agent.")
    complianceError: Optional[str] = Field(None, description="Error message if compliance check failed.")
...
async def _run_pre_screener(
    payslip_data: Optional[PayslipOutput], 
    bank_data: Optional[BankAnalyzerOutput],
    applicant_data: ApplicantData,
    kyc_data: KycData,
    rbi_income_guideline: float
) -> Tuple[str, str, Optional[Dict[str, Any]]]:
    """Agent 3: Analyzes combined structured data and generates a decision summary, including compliance."""
    ...
    # Existing logic for decision-making based on available data
    ...
    # NEW: Integrate Compliance Checker
    print("--- Running Compliance Checks ---")
    checker = ComplianceChecker()
    
    # Assuming submitted_documents can be inferred or passed separately
    # For now, let's assume all documents are submitted if payslip_data and bank_data are present
    submitted_documents = []
    if payslip_data: submitted_documents.append("Payslip")
    if bank_data: submitted_documents.append("Bank Statement")
    # Add other documents as needed, e.g., from an initial form submission
    required_documents = ["Aadhar Card", "PAN Card", "Bank Statement", "Payslip"]
    
    kyc_match, kyc_message = checker.check_kyc_mismatch(
        applicant_data.model_dump(), 
        kyc_data.model_dump()
    )
    
    docs_present, docs_message = checker.check_missing_documents(
        submitted_documents, 
        required_documents
    )
    
    income_meets_guideline, income_message = checker.check_income_guideline(
        applicant_data.income, 
        rbi_income_guideline
    )
    
    results = {
        "kyc_check": (kyc_match, kyc_message),
        "document_check": (docs_present, docs_message),
        "income_check": (income_meets_guideline, income_message),
    }
    
    compliance_report = checker.generate_compliance_report(results)

    if not compliance_report["overall_compliance"]:
        notes.append("COMPLIANCE FAILED: Review compliance report for details.")
        if status == "APPROVED": # If previously approved, change to pending review
            status = "PENDING_REVIEW"
            
    return status, " | ".join(notes), compliance_report
...
async def process_application(
    ...
    applicant_data_json: str = Field(..., description="JSON string of applicant's personal data."),
    kyc_data_json: str = Field(..., description="JSON string of applicant's KYC data."),
    rbi_income_guideline: float = Field(..., description="RBI income guideline for loan eligibility.")
):
    """
    ...
    try:
        applicant_data = ApplicantData(**json.loads(applicant_data_json))
        kyc_data = KycData(**json.loads(kyc_data_json))
    except json.JSONDecodeError as e:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"Invalid JSON for applicant or KYC data: {e}")
    except Exception as e:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"Error parsing applicant or KYC data: {e}")
    ...
    try:
        overall_status, pre_screening_notes, compliance_report = await _run_pre_screener(
            payslip_data, 
            bank_data, 
            applicant_data, 
            kyc_data, 
        ...
        pre_screening_notes = f"Pre-screener or Compliance Agent Failed: {e}"
        compliance_error = f"Compliance check could not be completed: {e}"
